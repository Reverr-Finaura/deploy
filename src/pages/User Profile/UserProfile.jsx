...
useEffect(()=>{
    ...
    fetchUserFundingDocFromFirebase()
      ...
      useEffect(()=>{
        if(user&&userDoc){
            if(userDoc?.hasGeneralProfile===true){setHasUserProfile(true);return}
            else if(userDoc?.hasGeneralProfile===false){setHasUserProfile(false)}
        }
        },[userDoc])
...
<section id='userProfilePage'>
    ...
    <section className='user-experience-info'>
        ...
        {userDoc?.experience?.length===0?<p className='no-social-link-avai-msg'>No Experience Details Available</p>:null}
...
<ul>
    ...
    <section className='user-education-info'>
        ...
        {userDoc?.education?.length===0?<p className='no-social-link-avai-msg'>No Education Details Available</p>:null}
    ...
    <h3 className='user-how-can-we-meet-title'>How Can We Meet</h3>
{userDoc?.instagramLink===""&&userDoc?.facebookLink===""&&userDoc?.twitterLink===""&&userDoc?.linkedinLink===""?<p className='no-social-link-avai-msg'>No Social Link Available</p>:null}
    <div className='user-how-can-we-meet-social-icon-cont'>
        {userDoc?.instagramLink===""?null:<div className='user-how-can-we-meet-social-icon'>
            <a href={userDoc?.instagramLink}><img src="./images/instaIcon.svg" alt="social-icon" /></a>
        </div>}


         {userDoc?.facebookLink===""?null:<div className='user-how-can-we-meet-social-icon'>
            <a href={userDoc?.facebookLink}><img src="./images/faceBookIcon.svg" alt="social-icon" /></a>
        </div>}
         {userDoc?.twitterLink===""?null:<div className='user-how-can-we-meet-social-icon'>
            <a href={userDoc?.twitterLink}><img src="./images/twitterIcon.svg" alt="social-icon" /></a>
        </div>}
         {userDoc?.linkedinLink===""?null:<div className='user-how-can-we-meet-social-icon'>
            <a href={userDoc?.linkedinLink}><img src="./images/linkedinIcon.svg" alt="social-icon" /></a>
        </div>}

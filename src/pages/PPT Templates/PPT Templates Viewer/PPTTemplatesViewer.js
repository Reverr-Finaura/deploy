...
useEffect(()=>{
  async function fetchPptLinkFromFirebase() {
    ...
    querySnapshot.forEach((doc) => {
      if(doc.id===pptId){setPPtLink(doc.data().link);setIsSideBarCalled(false)}
      })
  }
  ...
  return (
    ...
    {width >= 600 ? (
        <>
          <div style={{transform:isSideBarCalled?"translateX(0)":""}} className={styles.animatedSideBar}><BsArrowRightSquareFill onClick={()=>setIsSideBarCalled(e=>!e)} className={styles.arroww}/><SidebarFinal /></div>
          <NavBarFinal />
        </>
      ) : (
        <>

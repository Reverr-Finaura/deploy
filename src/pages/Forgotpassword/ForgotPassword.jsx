  ...
  const sendOtp = async(e) => {
    ...
    querySnapshot.forEach((doc) => {
      ...
      if (doc.id === email) {
        ...
        setTempUserData({name:doc.data().name,password:doc.data().password})
      }
    });
   if(JSON.stringify(tempDocData)==="{}"){toast.error("Email does not exist in database");setLoading(false);return}
      function generate(n) {
        var add = 1,
          max = 12 - add;
        if (n > max) {
...
const updatePasswordOfUser=async(e)=>{
  e.preventDefault()
  setLoading(true)
  if(newOtp!==tempOtp){toast.error("Wrong Otp");setLoading(false);return}
  if(password!==confirmPassword){toast.error("New Password and Confirm Password do no match");setLoading(false);return}
    ...
    <>
       <section className={styles.loginOuterCont}>
        ...
        <div className={styles.rightCont}>
           ...
           {!tempOtp&&<form onSubmit={sendOtp} className={styles.form}>
           <input
           className={styles.input}
                ...
                required
              />
               <button disabled={loading} className={styles.Button} type="submit">Send OTP</button>
              </form>}
             {
              tempOtp&&<form onSubmit={updatePasswordOfUser} className={styles.form}>
              <input
           className={styles.input}
           ...
           className={styles.input}
                ...
                required
              />
               <button style={{cursor:loading?"default":""}} disabled={loading} className={styles.Button} type="submit">UPDATE</button>
              </form>
             }

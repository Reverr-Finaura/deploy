const nonscoredData = {
  tech_used: [
    "3D Bioprinting",
    "3D Bioprinting Systems for Organ Transplant",
    "3D Printing",
    "3D Scanners",
    "4D Printing",
    "5G",
    "802.11 ax",
    "AI PaaS",
    "Activity Streams",
    "Advanced Analytics With Self-Service Delivery",
    "Affective Computing",
    "Application Stores",
    "Artificial General Intelligence",
    "Audio Mining/Speech Analytics",
    "Augmented Data Discovery",
    "Augmented Reality",
    "Automatic Content Recognition",
    "Autonomous Driving Level 4",
    "Autonomous Driving Level 5",
    "Autonomous Mobile Robots",
    "BYOD",
    "Big Data and Extreme Information Processing and Management",
    "Bioacoustic Sensing",
    "Biochips",
    "Biometric Authentication Methods",
    "Biotech - Cultured or Artificial Tissue",
    "Blockchain",
    "Blockchain for Data Security",
    "Brain-Computer Interface",
    "Carbon Nanotube",
    "Citizen Data Science",
    "Clean Tech",
    "Cloud Computing",
    "Cloud/Web Platforms",
    "Cognitive Computing",
    "Cognitive Expert Advisors",
    "Commercial UAVs (Drones)",
    "Complex Event Processing",
    "Connected Home",
    "Consumer 3D Printing",
    "Consumer Telematics",
    "Content Analytics",
    "Context Brokering",
    "Context-Enriched Services",
    "Conversational AI Platform",
    "Conversational User Interfaces",
    "Crowdsourcing",
    "Cryptocurrencies",
    "Cryptocurrency Exchange",
    "Data Broker PaaS",
    "Data Science",
    "Deep Learning",
    "Deep Neural Nets (Deep Learning)",
    "Deep Neural Network ASICs",
    "Deep Reinforcement Learning",
    "Democratization of Development Technology",
    "Digital Dexterity",
    "Digital Security",
    "Digital Twin",
    "Driverless Vehicles",
    "E-Book Readers",
    "Edge AI",
    "Edge Computing",
    "Electric Vehicles",
    "Electrovibration",
    "Enterprise 3D Printing",
    "Enterprise Taxonomy and Ontology Management",
    "Exoskeleton",
    "Flying Autonomous Vehicles",
    "Gamification",
    "General-Purpose Machine Intelligence",
    "Gesture Control Devices",
    "Gesture Recognition",
    "Group Buying",
    "HTML5",
    "Home Health Monitoring",
    "Hosted Virtual Desktops",
    "Human Augmentation",
    "Hybrid Cloud Computing",
    "Idea Management",
    "Image Recognition",
    "In-Memory Analytics",
    "In-Memory Database Management Systems",
    "Internet TV",
    "Internet of Things",
    "IoT Platform",
    "Knowledge Graphs",
    "Location Intelligence",
    "Location-Aware Applications",
    "Machine Learning",
    "Machine-to-Machine Communication Services",
    "Media Tablet",
    "Mesh-Networks: Sensor",
    "Micro Data Centers",
    "Mixed Reality",
    "Mobile Application Stores",
    "Mobile Health Monitoring",
    "Mobile OTA Payment",
    "Mobile Robots",
    "NFC",
    "NFC Payment",
    "Nanotube Electronics",
    "Natural Language Question Answering",
    "Neomorphic Hardware",
    "Neurobusiness",
    "Neuromorphic Hardware",
    "People Literate Technology",
    "Personal Analytics",
    "Predictive Analytics",
    "Prescriptive Analytics",
    "Private Cloud Computing",
    "QR/Colour Code",
    "Quantified Self",
    "Quantum Computing",
    "Self-healing System Technology",
    "Serverless PaaS",
    "Silicon Anode Batteries",
    "Smart Dust",
    "Smart Fabrics",
    "Smart Robots",
    "Smart Workspace",
    "Social Analytics",
    "Social TV",
    "Software-Defined Anything (SDx)",
    "Software-Defined Security",
    "Speech Recognition",
    "Speech to Speech Translation",
    "Text Analytics",
    "Video Analytics for Customer Service",
    "Virtual Assistants",
    "Virtual Personal Assistants",
    "Virtual Reality",
    "Virtual Worlds",
    "Volumetric & Holographic Displays",
    "Wearable User Interfaces",
    "Wearables",
    "Wireless Power",
  ],
  primary_offer: ["Product", "Service"],
  customer_segment: ["B2B", "D2C", "B2B2C", "C2C"],
  ismoat: ["Yes", "No"],
  certification: ["Yes", "No"],
  education: [
    "Lower than High-School",
    "High-School",
    "Undergraduate",
    "Post-Graduate / Masters",
    "PhD",
    "Post-Doctorate",
  ],
  personality: ["Same", "Few Similarities", "No Similarities"],
  exp_levels: [">=10 yrs", "5-10 yrs", "<5 yrs"],
  emotion_level: [
    "Stable - Bill Hewlett / David Packard",
    "Moderately Volatile - Bill Gates / Paul Allen",
    "Volatile - Jobs / Wozniak",
  ],
  team_apply: [
    "Each member operates independently and is an expert in his/her field",
    "High skillset/role overlap between 2 team members, remaining member different skillsets",
    "No dependence, no interchanging of roles",
    "The team leader acts as the primary channel of commmunication between the other two",
  ],
  industry: [
    "Advertising",
    "Aeronnautics Aerospace & Defense",
    "Agriculture & Allied Industries",
    "AI",
    "Airport Operations",
    "Analytics",
    "Animation",
    "Apparel and Textiles",
    "AR VR (Augmented + Virtual Reality)",
    "Architecture Interior Design",
    "Art & Photography",
    "Automotive",
    "Banking",
    "Biotechnology",
    "BPM",
    "Cement",
    "Chemicals",
    "Computer vision",
    "Construction",
    "Consumer Durables",
    "Dating matrimonial",
    "Design",
    "Edtech",
    "Education",
    "Electronics system design & Manufacturing ",
    "Enterprise software",
    "Engineering & Capital Goods",
    "Events",
    "Fashion",
    "Finance",
    "Fintech",
    "FMCG",
    "Food and beverages",
    "Government",
    "Gems & Jewellery",
    "Green Technology",
    "Healthcare & lifesciences",
    "House-Hold services",
    "Horticulture",
    "Human resources",
    " Indic languages startups",
    "InTravel and tourism",
    "Internet of things",
    "Insurance",
    "IT Services",
    "Logistics",
    "Manufacturing",
    "Marketing",
    "Media and entertainment",
    "Metals & Mining",
    "Nanotechnology",
    "Natural Sciences",
    "Non-Renewable energy",
    "NGO",
    "NPO",
    "Other speciality retailers",
    "Passenger experience",
    "Pets & animals",
    "Pharmaceuticals",
    "professional & Commercial services",
    "Real estate",
    "Renewable energy",
    "Robotics",
    "Rural development",
    "safety",
    "security solutions",
    "Social impact",
    "social network",
    "sports",
    "Technology hardware",
    "Telecommunications & Networking",
    "Toys and games",
    "Transportation & Storage",
    "Travel and tourism",
    "Waste management",
    "Others",
  ],
  domain: [
    "Brand & Public Relations",
    "Career Advice & Self Mastery",
    "Culture",
    "Data Science & Analytics",
    "Finance, Accounting & Controlling",
    "Human Resources & Recruiting",
    "Infrastructure & Security Leadership & Management",
    "Legal, Risk Management, Compliance",
    "Marketing & Growth",
    "Product Management",
    "Sales",
    "Social Entrepreneurship & Charity",
    "Software Engineering & Charity",
    "Startup: Founding & Funding",
    "Strategy & Business Development",
    "Sustainability",
    "Team Structure & Work Methodologies",
    "Transformation",
    "UX, Design & User Research",
  ],
  market_dmg: [
    "Target Gender",
    "Between Ages",
    "Earnings Between",
    "Location= Tier 1,2,3,4",
  ],
};

const scoredData = {
  motivation: [
    { value: "select", score: 0 },

    {
      value:
        "My domain experience in this space led me to realise this specific market opportunity",
      score: 8,
    },
    {
      value:
        "This solution represents an extension from my previous or family-backed venture",
      score: 9,
    },
    {
      value: "I thought up the solution myself when I experienced the problem",
      score: 8,
    },
    {
      value:
        "Being bitten by the entrepreneurial bug led me to not want to work in a corporate set-up",
      score: 2,
    },
    {
      value:
        "While not in my domain, I am good at spotting opportunities like this across markets",
      score: 4,
    },
  ],
  tech_based: [
    { value: "select", score: 0 },

    {
      value:
        "Yes - I am a tech professional with relevant experience to build the product",
      score: 10,
    },
    {
      value:
        "Yes- My co-founder is a tech professional with relevant experience",
      score: 9,
    },
    {
      value: "Yes- I will hire an in-house",
      score: 7,
    },
    {
      value: "No- I will outsource the tech developement completely",
      score: 5,
    },
  ],
  prod_stage: [
    { value: "select", score: 0 },

    {
      value: "1. Preliminary R&D, MVP not achieved, Pre-prototype stage",
      score: 5,
    },
    {
      value:
        "2. Initial R&D done, MVP achieved, Angel/Seed funding received (or sufficiently bootstrapped)",
      score: 6,
    },
    {
      value: "3. Past MVP Stage, First Generation Product/Offering completed",
      score: 7,
    },
    {
      value: "4. Initial Traction, early adopters investigate",
      score: 8,
    },
    {
      value:
        "5. Investigating product/market fit on larger scale, Media attention possibly received",
      score: 9,
    },
    {
      value:
        "6. Direct competitors discovered in the market, Series A round sought with VCs",
      score: 9,
    },
    {
      value: "7. Activity beyond early adopters",
      score: 10,
    },
    {
      value:
        "8. Technology starting to receive dissatisfaction/negative press from the market",
      score: 6,
    },
    {
      value: "9. Competitors starting to fail/merge",
      score: 5,
    },
  ],
  moat: [
    { value: "select", score: 0 },

    {
      value: "Barriers to Entry/Exit",
      score: 10,
    },
    {
      value: "Switching Cost",
      score: 9,
    },
    {
      value: "Network Effects",
      score: 8,
    },
    {
      value: "Economies of Scale",
      score: 7,
    },
    {
      value: "Intangible Property- Patent/Copyright/Trademark",
      score: 10,
    },
    {
      value: "I dont have a MOAT",
      score: 2,
    },
  ],
  pro_risk: [
    { value: "select", score: 0 },

    {
      value:
        "Yes, Business Model Risk- The company's business model is not viable and we may have to burn for a long time to aquire users",
      score: 6,
    },
    {
      value:
        "Yes, Market/Competition Risk- The market is heated up and is very competitive",
      score: 4,
    },
    {
      value:
        "Yes, Legal Risk- The product may have an intellectual property that has already been patented/ cpyrighted or it is exposed to any political influencce.",
      score: 5,
    },
    {
      value:
        "Yes, Macroeconomic Risk- The product is a commodity and depends largley on macroeconomic factors.",
      score: 6,
    },
    {
      value: "No",
      score: 10,
    },
  ],
  has: [
    { value: "select", score: 0 },
    { value: "Yes", score: 10 },
    { value: "No", score: 5 },
  ],
  year_of_exp: [
    { value: "select", score: 0 },

    { value: "10+", score: 10 },
    { value: "5-10", score: 9 },
    { value: "3-5", score: 8 },
    { value: "<3", score: 7 },
  ],
  working_years: [
    { value: "select", score: 0 },

    { value: "10+", score: 10 },
    { value: "5-10", score: 9 },
    { value: "3-5", score: 8 },
    { value: "<3", score: 7 },
  ],
  prod_launches: [
    { value: "select", score: 0 },

    { value: "5+", score: 10 },
    { value: "4", score: 9 },
    { value: "3", score: 9 },
    { value: "2", score: 8 },
    { value: "1", score: 8 },
    { value: "0", score: 7 },
  ],
  process_imp: [
    { value: "select", score: 0 },

    { value: "5+", score: 10 },
    { value: "4", score: 9 },
    { value: "3", score: 9 },
    { value: "2", score: 8 },
    { value: "1", score: 8 },
    { value: "0", score: 7 },
  ],
  new_team: [
    { value: "select", score: 0 },

    { value: "5+", score: 10 },
    { value: "4", score: 9 },
    { value: "3", score: 9 },
    { value: "2", score: 8 },
    { value: "1", score: 8 },
    { value: "0", score: 7 },
  ],
  dependencies: [
    { value: "select", score: 0 },

    { value: "High Dependence", score: 6 },
    { value: "Moderate", score: 8 },
    { value: "Low Dependence", score: 9 },
  ],
  informal_com: [
    { value: "select", score: 0 },

    {
      value: "High Levels (frequent get-togethers, parties, informal meetups)",
      score: 8,
    },
    {
      value: "Moderate Levels - balance of informal and formal interaction  ",
      score: 8,
    },
    {
      value:
        "Primarily formal communication - Need for teambonding exercises is apparent",
      score: 6,
    },
  ],
  skill_overlap: [
    { value: "select", score: 0 },

    {
      value: "Same Skillsets",
      score: 6,
    },
    {
      value: "Some Overlap",
      score: 8,
    },
    {
      value: "Zero Overlap",
      score: 9,
    },
  ],
  interaction: [
    { value: "select", score: 0 },

    {
      value:
        "Very necessary - the team's goals require that team members interact often",
      score: 7,
    },
    {
      value:
        "Somewhat necessary - significant parts of members' tasks can be performed individually",
      score: 8,
    },
    {
      value:
        "Minimal necessity - each member can manage their tasks almost completely independently",
      score: 9,
    },
  ],
  target_market: [
    { value: "select", score: 0 },

    { value: "B2B", score: 9 },
    { value: "B2C", score: 8 },
    { value: "B2B2C", score: 8 },
    { value: "C2C", score: 8 },
  ],
  growth_rate: [
    { value: "select", score: 0 },

    { value: "<5%", score: 6 },
    { value: "Between 5-10%", score: 7 },
    { value: "10-15%", score: 8 },
    { value: ">15%", score: 9 },
  ],
  revenue_stage: [
    { value: "select", score: 0 },

    { value: "Yes, We're generating Revenue for more than 1 year", score: 8 },
    { value: "Yes, We're generating Revenue but <1 Year", score: 7 },
    { value: "No We are pre-revenue", score: 6 },
  ],
  revenue_nature: [
    { value: "select", score: 0 },

    { value: "Recurring", score: 8 },
    { value: "One-time", score: 6 },
    { value: "Hybrid", score: 7 },
  ],
  mom: [
    { value: "select", score: 0 },

    { value: "<10%", score: 5 },
    { value: "Between 10 and 25%", score: 7 },
    { value: ">25%", score: 9 },
  ],
  qoq: [
    { value: "select", score: 0 },

    { value: "<10%", score: 5 },
    { value: "Between 10 and 25%", score: 7 },
    { value: ">25%", score: 9 },
  ],
  yoy: [
    { value: "select", score: 0 },

    { value: "<10%", score: 5 },
    { value: "Between 10 and 25%", score: 8 },
    { value: ">25%", score: 10 },
  ],
  curr_rate: [
    { value: "select", score: 0 },

    { value: "Zero", score: 10 },
    { value: "<10%", score: 8 },
    { value: "10%>>30%", score: 6 },
    { value: ">30%", score: 5 },
  ],
  ltv: [
    { value: "select", score: 0 },

    { value: "<3:1", score: 5 },
    { value: ">3:1", score: 10 },
  ],
  break_even: [
    { value: "select", score: 0 },

    { value: "Yes", score: 10 },
    { value: "No", score: 5 },
    { value: "-", score: 0 },
  ],
  est_break_even: [
    { value: "select", score: 0 },

    { value: "Within 6 months", score: 8 },
    { value: "6 months-1 year", score: 7 },
    { value: "More than 1 year", score: 6 },
  ],
  profitable: [
    { value: "select", score: 0 },

    { value: "Yes", score: 10 },
    { value: "No", score: 7 },
    { value: "-", score: 0 },
  ],
  gross_margin: [
    { value: "select", score: 0 },

    { value: "Negative", score: 4 },
    { value: "0<<40%", score: 6 },
    { value: "40%<<80%", score: 7 },
    { value: ">80%", score: 9 },
  ],
  net_margin: [
    { value: "select", score: 0 },

    { value: "Negative", score: 4 },
    { value: "0<<20%", score: 8 },
    { value: "20%<<40%", score: 9 },
    { value: ">40%", score: 10 },
  ],
  funds: [
    { value: "select", score: 0 },

    { value: "<6 Months", score: 6 },
    { value: "6<<12 Months", score: 7 },
    { value: ">12 Months", score: 8 },
  ],
  curr_ratio: [
    { value: "select", score: 0 },

    { value: ">2:1", score: 5 },
    { value: "<2:1", score: 10 },
  ],
  cash_ratio: [
    { value: "select", score: 0 },

    { value: ">1", score: 5 },
    { value: "<1", score: 10 },
  ],
  eqt_ratio: [
    { value: "select", score: 0 },

    { value: ">2:1", score: 5 },
    { value: "<2:1", score: 10 },
  ],
  financial_audited: [
    { value: "select", score: 0 },

    { value: "Yes", score: 10 },
    { value: "No", score: 5 },
  ],
};

export { scoredData, nonscoredData };
